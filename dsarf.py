...
class LSTM_obs(nn.Module):
    ...
    def __init__(self, D, rnn_dim, factor_dim, S):
        ...
        self.rnn = nn.LSTM(D, rnn_dim, 2, batch_first=False,
                           bidirectional=True)
        self.fc1_rnn = nn.Linear(2*rnn_dim, 2*rnn_dim)
        self.fc2_rnn = nn.Linear(2*rnn_dim, 2*rnn_dim)
        self.fc3_rnn = nn.Linear(2*rnn_dim, factor_dim*2)
        if S:
            self.fc1_rnn_s = nn.Linear(2*rnn_dim, 2*rnn_dim)
            self.fc2_rnn_s = nn.Linear(2*rnn_dim, 2*rnn_dim)
            self.fc3_rnn_s = nn.Linear(2*rnn_dim, S)
                ...
                if not self.dsarf.recursive_state:
                    ...
                    for i in range(T_b):
                        ...
                        q_s = (p_s.permute(1, 0)+1e-4).log()\
                              ...
                              -p_z_sig[:,:,i].sum(dim = -1)
                        q_s =  q_s.permute(1, 0)
                        s_t_1 = self.softmax(q_s)
                        q_s_t = torch.cat((q_s_t, q_s.unsqueeze(1)), dim = 1)
                    q_s_t_log = q_s_t.clone()
                    q_s_t = self.softmax(q_s_t)
